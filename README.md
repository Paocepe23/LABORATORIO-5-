...
El presente laboratorio se desarrolló en dos etapas complementarias con el objetivo de analizar la variabilidad de la frecuencia cardíaca (HRV) y el balance autonómico mediante procesamiento digital de señales ECG.
Primera etapa (Partes 1A, 1B y 1C): se trabajó con una señal ECG previamente adquirida mediante el dispositivo BITalino a una frecuencia de muestreo de 100 Hz, siguiendo el protocolo de 2 minutos en reposo seguidos de 2 minutos de lectura en voz alta. Sobre esta señal se aplicó el pre-procesamiento completo en Python, incluyendo el diseño e implementación del filtro IIR Butterworth pasa-banda de orden 4, la detección de picos R, el cálculo de intervalos R-R, el análisis de HRV en el dominio del tiempo y la construcción del diagrama de Poincaré con los índices CVI y CSI.
Segunda etapa (Partes 2A, 2B y 2C): se realizó la adquisición de señal ECG en tiempo real utilizando el sensor AD8232 conectado a un Arduino Uno, transmitiendo los datos por comunicación serial hacia Python en Spyder. Se repitió el mismo protocolo de grabación de 4 minutos, aplicando el filtro Butterworth en tiempo real muestra a muestra, y posteriormente se realizó el mismo análisis de segmentación, detección de picos R, HRV temporal y diagrama de Poincaré sobre la señal adquirida en vivo.
Esta estructura permitió comparar los resultados obtenidos con dos metodologías de adquisición distintas, validando la consistencia del análisis HRV independientemente del dispositivo utilizado.
## Parte 1A.
